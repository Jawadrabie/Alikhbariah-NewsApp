...
- دعم AR/EN لاحقاً.
 - إدخالContent ثنائي اللغة: كل محتوى يُدخَل عبر الـ Dashboard يجب أن يتوفر باللغتين العربية والإنجليزية (حقول `*_en` في قاعدة البيانات)، مع تحقق بسيط في واجهة الداشبورد لعدم قبول حفظ بدون إحدى اللغتين.
...
- نموذج خبر:
  ...
  - title_en
  - content_en
  - ملاحظة: واجهة إنشاء/تعديل الخبر في الداشبورد تطلب إدخال الحقول بالعربية والإنجليزية وإظهار تحقّق أساسي قبل الحفظ.
  - image upload
  - category
  - location
  - publish date
...
## 8.2 ملاحظة هامة حول الحقول النصية ثنائية اللغة
- جميع الحقول النصية التي تُعرض للمستخدم في التطبيق (مثل `title`, `content`, `broadcast_title`, `fallback_message`, `name`, `description`, `text` في الجداول المعنية) يجب أن تتوفر بنسخة إنجليزية مقابلة تنتهي بـ `_en` (مثال: `title_en`, `content_en`).
- يجب أن يدعم مخطط قاعدة البيانات والـ API إدخال كلتي النسختين من الداشبورد، مع استخدام النسخة الإنجليزية عند كانت لغة التطبيق `en` والنسخة العربية عند كانت `ar`، مع آلية fallback إلى النسخة العربية إن لم تتوفر الإنجليزية.
...
- لا يوجد قسم مستقل للشريط الإخباري في الداشبورد أو قاعدة البيانات، والاعتماد بالكامل على الأخبار العاجلة (`breaking_news`).
...
- تم التحقق من وجود وصول إداري فعّال إلى Supabase Database عبر مفتاح Service Role، وأصبح تنفيذ عمليات القراءة/التعديل على الجداول ممكنًا مباشرة من بيئة العمل.
