...
## 5.7 شاشة الفيديوهات
- تقسم الشاشة إلى محورين:
  1) **فئات الفيديو** (مثل: نشرات الأخبار، تقارير، مؤتمر صحفي...)  
    - تعرض كبطاقات أفقية/أقسام واضحة.
    - كل فئة تعرض فيديوهاتها فقط عبر `videos.category_id`.
  2) **البرامج** (مثل: إشراقة سورية، بتوقيت سوريا...)  
    - تعرض كقسم مستقل.
    - عند الضغط على برنامج يفتح شاشة حلقات البرنامج عبر `videos.program_id`.
- في كل عرض فيديو: صورة مصغرة + أيقونة تشغيل + عنوان + تاريخ.
- الترتيب الافتراضي: `published_at` ثم `created_at` من الأحدث للأقدم.
...
- **يرفع فقط**: عنوان الفيديو + رابط YouTube + (فئة فيديو اختيارية) + (برنامج اختياري).
- التطبيق يستخرج العرض عبر الرابط داخل واجهة الفيديو.
- خصائص إضافية: صورة غلاف اختيارية، تاريخ نشر، ترتيب، إخفاء مؤقت.
- القاعدة: يمكن ربط الفيديو بـ `category_id` أو `program_id` أو كليهما حسب نوع المحتوى.
...
### ضبط العلاقة بين الفيديوهات والفئات/البرامج
- من شاشة الفيديو في الداشبورد يجب توفير حقلي اختيار:
  - `video_category` (من categories حيث `type='video'`).
  - `program` (من جدول programs).
- منع ظهور فئات الأخبار العامة داخل اختيار فئات الفيديو.

## 7.9 إدارة الفئات والمواقع
- فئات + مواقع جغرافية.
- إضافة نوع الفئة عند الإنشاء/التعديل:
  - `news` لفئات الأخبار.
  - `video` لفئات الفيديو.
...
- type (text: `news` | `video`, default `news`)
...
## 8.4 SQL التنفيذ الخاصة بتقسيم الفيديو
- ملف التهيئة الأولي: `docs/supabase_init.sql`
- ملف التحديث الجديد: `docs/add_video_sections_schema.sql`
- بعد التنفيذ:
  - فئات الأخبار في التطبيق يجب أن تقرأ `categories` مع فلتر `type='news'`.
  - فئات الفيديو يجب أن تقرأ `categories` مع فلتر `type='video'`.
  - شاشة الفيديوهات/البرامج تعتمد على `videos.category_id` و `videos.program_id`.
...
- عند الضغط على حفظ:
  - نخزن نسخة خبر مبسطة محلياً (id/title/image/content/date/category/location).
...
- فئات الفيديو تعتمد على جدول categories حيث `type='video'`.
- البرامج تعتمد على جدول programs، وكل برنامج يحتوي حلقات من جدول videos عبر program_id.
- شاشة الفيديو العامة تعرض حسب الفئة، بينما شاشة البرنامج تعرض حلقات البرنامج فقط.
...
- الخبر يتكوّن من: title + content فقط (بدون summary في قاعدة البيانات والتطبيق).
