...
select
  ...
  (select id from public.categories where slug = 'video-reports' and coalesce(type, 'news') = 'video' limit 1),
  1,
  false
where exists (select 1 from public.programs where name = 'عين على الحدث')
  and exists (select 1 from public.categories where slug = 'video-reports' and coalesce(type, 'news') = 'video');
